 ...
 *
 */
class CF7GSC_Google_ServiceResource {
  ...
  /** @var CF7GSC_Google_Service $service */
  private $service;
   ...
   * @param $name
   * @param $arguments
   * @return CF7GSC_Google_HttpRequest|array
   * @throws CF7GSC_Google_Exceptions
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new CF7GSC_Google_Exceptions("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new CF7GSC_Google_Exceptions("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new CF7GSC_Google_Exceptions("($name) missing required param: '$paramName'");
        }
    ...
    if (isset($method['mediaUpload'])) {
      $media = CF7GSC_Google_MediaFileUpload::process($postBody, $parameters);
      if ($media) {
    ...
    $url = CF7GSC_Google_REST::createRequestUri($servicePath, $method['path'], $parameters);
    $httpRequest = new CF7GSC_Google_HttpRequest($url, $method['httpMethod'], null, $postBody);
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = CF7GSC_Google_Utils::getStrLen($postBody);
      }
    ...
    return CF7GSC_Google_REST::execute($httpRequest);
  }
