...
class CF7GSC_Google_MediaFileUpload {
  ...
  public static function getUploadType($meta, &$payload, &$params) {
    if (isset($params['mediaUpload'])
        && get_class($params['mediaUpload']['value']) == 'CF7GSC_Google_MediaFileUpload') {
  ...
  public function nextChunk(CF7GSC_Google_HttpRequest $req) {
    ...
    $httpRequest = new CF7GSC_Google_HttpRequest($this->resumeUri, 'PUT', $headers, $data);
    ...
    if (308 == $code) {
      ...
      return CF7GSC_Google_REST::decodeHttpResponse($response);
    }
  }

  private function getResumeUri(CF7GSC_Google_HttpRequest $httpRequest) {
    $result = null;
    $body = $httpRequest->getPostBody();
    if ($body) {
      $httpRequest->setRequestHeaders(array(
        'content-type' => 'application/json; charset=UTF-8',
        'content-length' => CF7GSC_Google_Utils::getStrLen($body),
    ...
    throw new CF7GSC_Google_Exceptions("Failed to start the resumable upload");
  }
}