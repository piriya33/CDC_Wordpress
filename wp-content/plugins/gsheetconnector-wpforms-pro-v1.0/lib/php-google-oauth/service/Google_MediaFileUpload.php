...
class WPFGSC_Google_MediaFileUpload {
  ...
  public static function getUploadType($meta, &$payload, &$params) {
    if (isset($params['mediaUpload'])
        && get_class($params['mediaUpload']['value']) == 'WPFGSC_Google_MediaFileUpload') {
  ...
  public function nextChunk(WPFGSC_Google_HttpRequest $req) {
    ...
    $httpRequest = new WPFGSC_Google_HttpRequest($this->resumeUri, 'PUT', $headers, $data);
    $response = WPFGSC_Google_Client::$io->authenticatedRequest($httpRequest);
    ...
    if (308 == $code) {
      ...
      return WPFGSC_Google_REST::decodeHttpResponse($response);
    }
  }

  private function getResumeUri(WPFGSC_Google_HttpRequest $httpRequest) {
    $result = null;
    $body = $httpRequest->getPostBody();
    if ($body) {
      $httpRequest->setRequestHeaders(array(
        'content-type' => 'application/json; charset=UTF-8',
        'content-length' => WPFGSC_Google_Utils::getStrLen($body),
        'x-upload-content-type' => $this->mimeType,
        'expect' => '',
      ));
    }

    $response = WPFGSC_Google_Client::$io->makeRequest($httpRequest);
    $location = $response->getResponseHeader('location');
    $code = $response->getResponseHttpCode();
    if (200 == $code && true == $location) {
      return $location;
    }
    throw new WPFGSC_Google_Exceptions("Failed to start the resumable upload");
  }
}