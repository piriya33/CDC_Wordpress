 ...
 *
 */
class WPFGSC_Google_ServiceResource {
  ...
  /** @var WPFGSC_Google_Service $service */
  private $service;
   ...
   * @param $name
   * @param $arguments
   * @return WPFGSC_Google_HttpRequest|array
   * @throws WPFGSC_Google_Exceptions
   */
  public function __call($name, $arguments) {
    if (! isset($this->methods[$name])) {
      throw new WPFGSC_Google_Exceptions("Unknown function: {$this->serviceName}->{$this->resourceName}->{$name}()");
    }
    ...
    foreach ($parameters as $key => $val) {
      if ($key != 'postBody' && ! isset($method['parameters'][$key])) {
        throw new WPFGSC_Google_Exceptions("($name) unknown parameter: '$key'");
      }
    }
    if (isset($method['parameters'])) {
      foreach ($method['parameters'] as $paramName => $paramSpec) {
        if (isset($paramSpec['required']) && $paramSpec['required'] && ! isset($parameters[$paramName])) {
          throw new WPFGSC_Google_Exceptions("($name) missing required param: '$paramName'");
        }
    ...
    if (isset($method['mediaUpload'])) {
      $media = WPFGSC_Google_MediaFileUpload::process($postBody, $parameters);
      if ($media) {
    ...
    $url = WPFGSC_Google_REST::createRequestUri($servicePath, $method['path'], $parameters);
    $httpRequest = new WPFGSC_Google_HttpRequest($url, $method['httpMethod'], null, $postBody);
    if ($postBody) {
      ...
      if (isset($contentType) && $contentType) {
        ...
        $contentTypeHeader['content-length'] = WPFGSC_Google_Utils::getStrLen($postBody);
      }
      $httpRequest->setRequestHeaders($contentTypeHeader);
    }

    $httpRequest = WPFGSC_Google_Client::$auth->sign($httpRequest);
    if (WPFGSC_Google_Client::$useBatch) {
      return $httpRequest;
    }
    ...
    return WPFGSC_Google_REST::execute($httpRequest);
  }
