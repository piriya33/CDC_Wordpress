   ...
   jQuery(document).on('click', '#save-wpform-gs-code', function (event) {
      ...
      jQuery.post(ajaxurl, data, function (response) {
         if (!response.success) {
            ...
            jQuery(".loading-sign-btn").removeClass("loading");
            jQuery("#wp-validation-message").empty();
            jQuery("<span class='wp-valid-message'>Your Google Access Code is Authorized and Saved.</span> <br/><br/><span class='wp-valid-notice'> Note: If you are getting any errors or not showing sheet in dropdown, then make sure to check the debug log. To contact us for any issues do send us your debug log.</span>").appendTo('#wp-validation-message');
         }
      });
