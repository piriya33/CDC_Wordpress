# The Events Calendar Pro PostCSS Styles

## Class naming consistency and BEM

A couple of issues we've had previously with templates for Modern Tribe plugins was inconsistent class naming and the class naming structure. To deal with this, we've adopted the use of [BEM](http://getbem.com/naming/) for class naming, combined with the use of `tribe-events-pro-` as a block prefix.
...
Secondly, we've added prefixes to our classes. The first prefix we've used is `tribe-events-pro-`. This is mainly to avoid styles clashing with other theme styles. For example, if we used a class `header`, a theme that the user may apply may also use a class `header` and the theme styles may unintentionally affect the plugin styles. Instead, we use `tribe-events-pro-header`. The second prefix we've used is context-based prefixes. Within The Events Calendar, we mainly use `l-` for layout and `c-` for component. Modern Tribe's Common plugin uses more context-based prefixes. These prefixes help determine the context of these reusable style classes. For example, the class `tribe-events-c-search` can be applied, along with the element classes, to a search component to apply search component styles.

## View/block wrapper class

In order to not override theme styles and elements outside of Modern Tribe plugins, we've added a wrapper class `tribe-events-pro` around all of Modern Tribe plugins blocks and views. For example, the markup for a specific view or block might look like the following:

```
<div class="tribe-events-pro">
	...
	<div class="tribe-events-pro-photo__event">
		...
	</div>
	...
</div>
...
We need the `tribe-events-pro` wrapper class in order to override Common resets and styles, as they use the `tribe-common` wrapper class. This also allows us to target only the elements we intend to target within the Modern Tribe plugin views while reducing the probability of clashing styles with themes.
...
In this case, the event details is an element of the map view. However, the `--has-thumbnail` modifier is applied to the top level element event card wrapper. Given this structure, our minimum specificity becomes 3 classes.
...
## Structure of The Events Calendar Pro styles

The Events Calendar Pro styles are comprised of 2 files: `views-skeleton.pcss` and `views-full.pcss`. The views skeleton styles cover basic layout styles for the views and components. The views full styles, combined with the skeleton styles, make up the full suite of styles for The Events Calendar Pro plugin views.

The Events Calendar Pro styles are broken into 3 main sections: utilities, components, and views.
...
The views styles are styles that are specific to each view and the sub-elements of each view. These are generally more layout-focused after applying common styles, though some elements, such as in the week view, require more custom styles.
