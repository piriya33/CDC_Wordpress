...
/*
 * Utilities class for wpform google sheet connector pro
 * @since       1.0
 */
...
/**
 * Wpform_gs_Connector_Utility class - singleton class
 * @since 1.0
 */
class Wpform_gs_Connector_Utility {
    ...
    * @since 1.0
    */
   public static function wpform_debug_log($error) {
      try {
         if (!is_dir(WPFORMS_GOOGLESHEET_PRO_PATH . 'logs')) {
            mkdir(WPFORMS_GOOGLESHEET_PRO_PATH . 'logs', 0755, true);
         }
      } catch (Exception $e) {
         
      }
      try {
         $log = fopen(WPFORMS_GOOGLESHEET_PRO_PATH . "includes/logs/log.txt", 'a');
         if (is_array($error)) {
