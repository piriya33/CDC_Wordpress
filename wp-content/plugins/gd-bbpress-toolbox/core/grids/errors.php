...
class gdbbx_grid_errors extends d4p_grid {
    ...
    public function column_file($item){
        $actions = array(
            'delete' => sprintf('<a href="admin.php?page=gd-bbpress-toolbox-errors&single-action=%s&error=%s&_wpnonce=%s">%s</a>', 'delete', $item->meta_id, wp_create_nonce('gd-bbpress-toolbox-error'), __("Delete", "gd-bbpress-toolbox"))
        );

        $type = $this->attachment_type($item);

        $render = !empty($type) ? $type.': ' : '';
        $render.= '<strong>'.$item->error['file'].'</strong>';
